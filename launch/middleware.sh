...
print_help() {
  ...
  echo -e "  # 前台启动中间件服务"
  echo -e "  $ ./middleware.sh\n"
  echo -e "  # 后台启动中间件服务"
  echo -e "  $ ./middleware.sh -d\n"
  echo -e "  # 快速停止中间件容器"
  echo -e "  $ ./middleware.sh -s\n"
  echo -e "  # 关闭并移除中间件容器"
  echo -e "  $ ./middleware.sh -e\n"
  echo -e "  # 显示帮助信息"
  echo -e "  $ ./middleware.sh -h"
  exit 0
}
...
# 设置要启动的中间件服务
MIDDLEWARE_SERVICES="network-service postgresql minio redis elasticsearch mosquitto casdoor"

if [ "${EXIT_COMMAND}" -eq 1 ]; then
  echo -e "${BRIGHT_YELLOW}▶  退出进程 ${RESET}"
  docker compose "${CMD_ARGS[@]}" down ${MIDDLEWARE_SERVICES}
  exit
fi

if [ "${STOP_COMMAND}" -eq 1 ]; then
  echo -e "${BRIGHT_YELLOW}▶  停止容器 ${RESET}"
  docker compose "${CMD_ARGS[@]}" stop ${MIDDLEWARE_SERVICES}
  exit
fi

echo -e "${BRIGHT_CYAN}▶ 仅启动中间件服务: ${MIDDLEWARE_SERVICES} ${RESET}"

if [ "${BACKGROUND_MODE}" -eq 1 ]; then
  echo -e "${BRIGHT_YELLOW}▶ 以守护进程模式启动${RESET}"
  docker compose "${CMD_ARGS[@]}" up -d ${MIDDLEWARE_SERVICES}
else
  echo -e "${BRIGHT_YELLOW}▶ 以前台模式启动${RESET}"
  docker compose "${CMD_ARGS[@]}" up ${MIDDLEWARE_SERVICES}
fi
