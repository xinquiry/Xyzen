...
export const ChartRenderer: React.FC<ChartRendererProps> = ({
    ...
    try {
      ...
      if (hasEChartsReadyData) {
        ...
        const series = seriesData.map((series) => {
          ...
          const seriesType =
            config.type === "area" || isStackedArea
              ? ("line" as const)
              : config.type;
          ...
          if (seriesType === "line") {
            return {
              ...
              areaStyle:
                series.areaStyle ||
                (config.type === "area" || isStackedArea ? {} : undefined),
        ...
        const categories =
          config.xAxis?.type === "category"
            ? seriesData[0]?.data.map((point) => point.x) || []
            : undefined;
      ...
      if (config.options) {
        ...
        if (
          ...
          const seriesOverrides = config.options.series as Record<
            string,
            unknown
          >;
          ...
          option.series = option.series.map((series) => {
            ...
            if (config.type === "scatter") {
              ...
              if (
                seriesOverrides.itemStyle &&
                typeof seriesOverrides.itemStyle === "object"
              ) {
                updatedSeries.itemStyle = {
                  ...((updatedSeries.itemStyle as Record<string, unknown>) ||
                    {}),
                  ...seriesOverrides.itemStyle,
                };
              }
              if (
                seriesOverrides.emphasis &&
                typeof seriesOverrides.emphasis === "object"
              ) {
                updatedSeries.emphasis = {
                  ...((updatedSeries.emphasis as Record<string, unknown>) ||
                    {}),
                  ...seriesOverrides.emphasis,
                };
              }
              ...
              if (
                seriesOverrides.lineStyle &&
                typeof seriesOverrides.lineStyle === "object"
              ) {
                updatedSeries.lineStyle = {
                  ...((updatedSeries.lineStyle as Record<string, unknown>) ||
                    {}),
                  ...seriesOverrides.lineStyle,
                };
              }
              ...
              if (
                seriesOverrides.areaStyle &&
                typeof seriesOverrides.areaStyle === "object"
              ) {
                updatedSeries.areaStyle = {
                  ...((updatedSeries.areaStyle as Record<string, unknown>) ||
                    {}),
                  ...seriesOverrides.areaStyle,
                };
              }
  ...
  if (!isValidOption || !hasRequiredProperties) {
    return (
      <div
        className={clsx("chart-renderer", className)}
        style={{ height, width }}
      >
        <div
        ...
        >
          ...
          <div className="text-center max-w-sm">
            ...
            <p className="text-red-600 dark:text-red-400 text-sm leading-relaxed">
              Unable to render chart data. The data format may be invalid or
              unsupported.
            </p>
          </div>
          ...
          {process.env.NODE_ENV === "development" && (
            <details className="mt-4 max-w-md">
              ...
              <pre className="mt-2 text-xs text-red-600 bg-red-100 dark:bg-red-900/20 p-2 rounded overflow-auto max-h-32">
                {JSON.stringify(
                  { isValidOption, hasRequiredProperties, data },
                  null,
                  2,
                )}
              </pre>
  ...
  if (isSeriesData) {
    ...
    return {
      xAxis: {
        ...
        data:
          config.labels ||
          seriesData[0].data.map((_, index) => index.toString()),
        name: config.xAxis?.name,
      },
      yAxis: {
        type: "value",
      ...
      series: seriesData.map((series) => {
        const seriesType =
          series.type || (config.type === "area" ? "line" : config.type);
        const baseConfig = {
          name: series.name,
          data: series.data,
...
function generateScatterChart(
  ...
  return {
    ...
    tooltip: {
      ...
      formatter:
        "{b0}: {c0}" +
        (config.yAxis?.name ? `<br/>${config.yAxis.name}: {c1}` : ""),
    },
  };
}
...
function generateHeatmapChart(
  ...
  if (Array.isArray(config.data)) {
    ...
    if (
      ...
      const dataPoints = (config.data as unknown[]).filter(
        (item): item is { x: string; y: string; value: number } => {
          if (
            typeof item !== "object" ||
            item === null ||
            !("x" in item) ||
      ...
      const matrix = (config.data as unknown[]).filter(
        (item): item is number[] =>
      ...
      const dataPoints = (config.data as unknown[]).filter(
        (
          item,
        ): item is { x: string | number; y: string | number; value?: number } =>
          typeof item === "object" &&
          item !== null &&
          "x" in item &&
          "y" in item,
...
function mergeOptions(
  base: EChartsOption,
  override: Partial<EChartsOption>,
): EChartsOption {
  const merged = { ...base };
