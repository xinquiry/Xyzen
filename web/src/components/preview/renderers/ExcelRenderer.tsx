...
export const ExcelRenderer = ({ file, url, className }: RendererProps) => {
  // Always use PDF conversion for Excel files
  return (
  ...
  React.useEffect(() => {
    const convertAndPreview = async () => {
      try {
        ...
        // Call the backend API to convert XLSX to PDF
        const convertPath = `/xyzen/api/v1/files/${file.id}/xlsx-to-pdf`;
    ...
    return (
      ...
      >
        <div className="text-center text-white">
          ...
          <p className="mb-2">Excel 转换为 PDF 中...</p>
          <p className="text-sm text-neutral-400">文件较大，正在转换电子表格</p>
        </div>
      </div>
    );
  }
