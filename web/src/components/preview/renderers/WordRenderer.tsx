...
export const WordRenderer = ({ file, url, className }: RendererProps) => {
  // Always use PDF conversion for Word files
  return (
  ...
  React.useEffect(() => {
    const convertAndPreview = async () => {
      try {
        ...
        // Call the backend API to convert DOCX to PDF
        const convertPath = `/xyzen/api/v1/files/${file.id}/docx-to-pdf`;
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to convert Word file: ${response.status} ${response.statusText}`,
          );
        }
        ...
        console.error("Failed to convert Word to PDF:", err);
        setError(
    ...
    return (
      ...
      >
        <div className="text-center text-white">
          ...
          <p className="mb-2">Word 转换为 PDF 中...</p>
          <p className="text-sm text-neutral-400">文件较大，正在转换文档</p>
        </div>
      </div>
    );
  }
