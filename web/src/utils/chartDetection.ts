    ...
    // Direct ECharts configuration (highest confidence)
    {
      ...
      test: (data: unknown) => {
        return (
          ...
          data !== null &&
          ("echarts" in data ||
            ("xAxis" in data && "yAxis" in data && "series" in data))
        );
      },
      ...
      transform: (data: unknown) => {
        ...
        return {
          ...
          data: points.map((point) => {
            ...
            return {
              x: (point.timestamp || pointRecord.time || pointRecord.date) as
                | string
                | number,
              y: (point.value ||
    ...
    // Array of coordinate objects
    {
      ...
      transform: (data: unknown) => {
        ...
        return {
          ...
          data: points,
          xAxis: {
            type: isNumericSeries(points.map((p) => p.x))
              ? "value"
              : "category",
          },
          yAxis: { type: "value" },
    ...
    // Array of objects with numeric values (table-like data)
    {
      ...
      test: (data: unknown) => {
        ...
        const numericKeys = Object.keys(firstItem).filter(
          (key) => typeof firstItem[key as keyof typeof firstItem] === "number",
        );
...
function extractChartType(
  ...
  if (normalizedHint.includes("bar") || normalizedHint.includes("column"))
    return "bar";
  if (normalizedHint.includes("pie") || normalizedHint.includes("donut"))
    return "pie";
  if (normalizedHint.includes("scatter") || normalizedHint.includes("point"))
    return "scatter";
  if (normalizedHint.includes("area")) return "area";
  if (normalizedHint.includes("heatmap") || normalizedHint.includes("heat"))
    return "heatmap";
