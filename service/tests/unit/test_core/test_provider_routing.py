...
class TestProviderRouting:
    @pytest.mark.asyncio
    async def test_system_fallback_routes_gpt_models_to_azure_openai(self, monkeypatch: MonkeyPatch) -> None:
        """When provider_id is missing, gpt-* should not accidentally route to Vertex."""

        # Mock ModelsDevService.get_model_info_for_key to avoid network calls
        async def _fake_get_model_info(_model: str, _provider: str | None = None) -> ModelInfo:
            return ModelInfo(key=_model, litellm_provider="openai")

        monkeypatch.setattr(
            "app.core.providers.manager.ModelsDevService.get_model_info_for_key",
        ...
        await manager.create_langchain_model(provider_id=None, model="gpt-5.2")
